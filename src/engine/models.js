// Comprehensive database of open LLMs for developers.
// All benchmark numbers, RAM requirements, and specs are realistic estimates
// based on publicly available information as of April 2026.

export const MODELS = [
  // ── DeepSeek Coder V2 ──────────────────────────────────────────────
  {
    id: "deepseek-coder-v2-16b",
    legacy: true,
    name: "DeepSeek Coder V2 Lite",
    family: "DeepSeek Coder V2",
    params: "16B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "instruction-following", "math", "reasoning", "fill-in-middle"],
    weaknesses: ["multilingual-prose", "creative-writing"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 73.8, swebench: 15.2, aiderPolyglot: 18.4 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-06-17",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-Coder-V2-Lite-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "deepseek-coder-v2-236b",
    legacy: true,
    name: "DeepSeek Coder V2",
    family: "DeepSeek Coder V2",
    params: "236B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "reasoning", "math", "instruction-following", "fill-in-middle", "architecture"],
    weaknesses: ["compute-heavy"],
    minRAM: "128GB",
    typicalLatency: "high",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 90.2, swebench: 22.6, aiderPolyglot: 35.1 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-06-17",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-Coder-V2-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },

  // ── DeepSeek V3 ────────────────────────────────────────────────────
  {
    id: "deepseek-v3",
    legacy: true,
    name: "DeepSeek V3",
    family: "DeepSeek V3",
    params: "671B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "reasoning", "math", "instruction-following", "multilingual", "architecture"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "256GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 91.6, swebench: 26.8, aiderPolyglot: 49.6 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-12-26",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-V3",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },

  // ── CodeLlama ──────────────────────────────────────────────────────
  {
    id: "codellama-7b",
    name: "CodeLlama 7B",
    family: "CodeLlama",
    params: "7B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "instruction-following"],
    weaknesses: ["reasoning", "architecture", "multilingual"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 33.5, swebench: 2.1, aiderPolyglot: 5.8 },
    defaultConfig: { temperature: 0.1, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2023-08-24",
    provider: "Meta",
    url: "https://huggingface.co/codellama/CodeLlama-7b-Instruct-hf",
    legacy: true,
    enterprise: {
      managedHosting: ["together.ai", "replicate", "fireworks.ai", "aws-bedrock"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "codellama-13b",
    name: "CodeLlama 13B",
    family: "CodeLlama",
    params: "13B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "instruction-following"],
    weaknesses: ["reasoning", "architecture", "multilingual"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 36.0, swebench: 3.4, aiderPolyglot: 7.2 },
    defaultConfig: { temperature: 0.1, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2023-08-24",
    provider: "Meta",
    url: "https://huggingface.co/codellama/CodeLlama-13b-Instruct-hf",
    legacy: true,
    enterprise: {
      managedHosting: ["together.ai", "replicate", "fireworks.ai", "aws-bedrock"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "codellama-34b",
    name: "CodeLlama 34B",
    family: "CodeLlama",
    params: "34B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "reasoning", "instruction-following"],
    weaknesses: ["architecture", "multilingual"],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 48.8, swebench: 6.2, aiderPolyglot: 11.5 },
    defaultConfig: { temperature: 0.1, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2023-08-24",
    provider: "Meta",
    url: "https://huggingface.co/codellama/CodeLlama-34b-Instruct-hf",
    legacy: true,
    enterprise: {
      managedHosting: ["together.ai", "replicate", "fireworks.ai", "aws-bedrock"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "codellama-70b",
    name: "CodeLlama 70B",
    family: "CodeLlama",
    params: "70B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "reasoning", "instruction-following"],
    weaknesses: ["architecture", "multilingual", "compute-heavy"],
    minRAM: "64GB",
    typicalLatency: "high",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["vllm", "huggingface", "llama.cpp"],
    deploymentOptions: ["cloud", "hybrid"],
    benchmarks: { humaneval: 53.0, swebench: 8.5, aiderPolyglot: 14.3 },
    defaultConfig: { temperature: 0.1, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-01-29",
    provider: "Meta",
    url: "https://huggingface.co/codellama/CodeLlama-70b-Instruct-hf",
    legacy: true,
    enterprise: {
      managedHosting: ["together.ai", "replicate", "fireworks.ai", "aws-bedrock"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── StarCoder2 ─────────────────────────────────────────────────────
  {
    id: "starcoder2-3b",
    name: "StarCoder2 3B",
    family: "StarCoder2",
    params: "3B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "multilingual"],
    weaknesses: ["reasoning", "instruction-following", "architecture"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 31.7, swebench: 1.8, aiderPolyglot: 4.9 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-02-28",
    provider: "BigCode",
    url: "https://huggingface.co/bigcode/starcoder2-3b",
    legacy: true,
    enterprise: {
      managedHosting: ["huggingface-endpoints"],
      sdkQuality: 3,
      communitySize: 3,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "starcoder2-7b",
    name: "StarCoder2 7B",
    family: "StarCoder2",
    params: "7B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "multilingual"],
    weaknesses: ["reasoning", "instruction-following", "architecture"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 35.4, swebench: 2.5, aiderPolyglot: 6.4 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-02-28",
    provider: "BigCode",
    url: "https://huggingface.co/bigcode/starcoder2-7b",
    legacy: true,
    enterprise: {
      managedHosting: ["huggingface-endpoints"],
      sdkQuality: 3,
      communitySize: 3,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "starcoder2-15b",
    name: "StarCoder2 15B",
    family: "StarCoder2",
    params: "15B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "multilingual"],
    weaknesses: ["reasoning", "architecture"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 46.3, swebench: 5.1, aiderPolyglot: 9.7 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-02-28",
    provider: "BigCode",
    url: "https://huggingface.co/bigcode/starcoder2-15b",
    legacy: true,
    enterprise: {
      managedHosting: ["huggingface-endpoints"],
      sdkQuality: 3,
      communitySize: 3,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },

  // ── Llama 3.1 ──────────────────────────────────────────────────────
  {
    id: "llama-3.1-8b",
    legacy: true,
    name: "Llama 3.1 8B Instruct",
    family: "Llama 3.1",
    params: "8B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "multilingual", "codegen"],
    weaknesses: ["specialized-code-tasks"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 62.2, swebench: 10.1, aiderPolyglot: 15.6 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-07-23",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "llama-3.1-70b",
    legacy: true,
    name: "Llama 3.1 70B Instruct",
    family: "Llama 3.1",
    params: "70B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "multilingual", "codegen", "architecture", "math"],
    weaknesses: ["compute-heavy"],
    minRAM: "64GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["cloud", "hybrid"],
    benchmarks: { humaneval: 80.5, swebench: 18.3, aiderPolyglot: 30.2 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-07-23",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Meta-Llama-3.1-70B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "llama-3.1-405b",
    legacy: true,
    name: "Llama 3.1 405B Instruct",
    family: "Llama 3.1",
    params: "405B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "multilingual", "codegen", "architecture", "math"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "256GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 89.0, swebench: 23.1, aiderPolyglot: 40.5 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-07-23",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Meta-Llama-3.1-405B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── Llama 3.2 (small/on-device) ───────────────────────────────────
  {
    id: "llama-3.2-1b",
    legacy: true,
    name: "Llama 3.2 1B Instruct",
    family: "Llama 3.2",
    params: "1B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "on-device", "low-latency"],
    weaknesses: ["codegen", "reasoning", "architecture"],
    minRAM: "2GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "hybrid"],
    benchmarks: { humaneval: 14.6, swebench: null, aiderPolyglot: 1.2 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 2048 },
    releaseDate: "2024-09-25",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Llama-3.2-1B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "llama-3.2-3b",
    legacy: true,
    name: "Llama 3.2 3B Instruct",
    family: "Llama 3.2",
    params: "3B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "on-device", "low-latency", "codegen"],
    weaknesses: ["reasoning", "architecture"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "hybrid"],
    benchmarks: { humaneval: 25.6, swebench: 1.4, aiderPolyglot: 3.8 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 2048 },
    releaseDate: "2024-09-25",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Llama-3.2-3B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── Mistral / Mixtral ──────────────────────────────────────────────
  {
    id: "mistral-7b",
    name: "Mistral 7B Instruct v0.3",
    family: "Mistral",
    params: "7B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "codegen", "multilingual"],
    weaknesses: ["specialized-code-tasks", "architecture"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 40.2, swebench: 4.8, aiderPolyglot: 9.1 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-05-22",
    provider: "Mistral AI",
    url: "https://huggingface.co/mistralai/Mistral-7B-Instruct-v0.3",
    legacy: true,
    enterprise: {
      managedHosting: ["mistral-platform", "together.ai", "fireworks.ai", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "mixtral-8x7b",
    name: "Mixtral 8x7B Instruct",
    family: "Mixtral",
    params: "46.7B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "codegen", "multilingual", "math"],
    weaknesses: ["memory-footprint"],
    minRAM: "48GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["cloud", "hybrid"],
    benchmarks: { humaneval: 45.1, swebench: 7.2, aiderPolyglot: 12.8 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-01-08",
    provider: "Mistral AI",
    url: "https://huggingface.co/mistralai/Mixtral-8x7B-Instruct-v0.1",
    legacy: true,
    enterprise: {
      managedHosting: ["mistral-platform", "together.ai", "fireworks.ai", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "mixtral-8x22b",
    name: "Mixtral 8x22B Instruct",
    family: "Mixtral",
    params: "141B",
    contextWindow: 65_536,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "codegen", "multilingual", "math", "architecture"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "128GB",
    typicalLatency: "high",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 55.8, swebench: 10.4, aiderPolyglot: 18.9 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-04-17",
    provider: "Mistral AI",
    url: "https://huggingface.co/mistralai/Mixtral-8x22B-Instruct-v0.1",
    legacy: true,
    enterprise: {
      managedHosting: ["mistral-platform", "together.ai", "fireworks.ai", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── CodeGemma ──────────────────────────────────────────────────────
  {
    id: "codegemma-2b",
    name: "CodeGemma 2B",
    family: "CodeGemma",
    params: "2B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "on-device"],
    weaknesses: ["reasoning", "instruction-following", "architecture", "multilingual"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "hybrid"],
    benchmarks: { humaneval: 31.1, swebench: null, aiderPolyglot: 3.2 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-04-09",
    provider: "Google",
    url: "https://huggingface.co/google/codegemma-2b",
    legacy: true,
    enterprise: {
      managedHosting: ["google-vertex-ai"],
      sdkQuality: 4,
      communitySize: 3,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "codegemma-7b",
    name: "CodeGemma 7B Instruct",
    family: "CodeGemma",
    params: "7B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "fill-in-middle", "instruction-following"],
    weaknesses: ["reasoning", "architecture", "multilingual"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 44.4, swebench: 4.2, aiderPolyglot: 8.1 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-04-09",
    provider: "Google",
    url: "https://huggingface.co/google/codegemma-7b-it",
    legacy: true,
    enterprise: {
      managedHosting: ["google-vertex-ai"],
      sdkQuality: 4,
      communitySize: 3,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── Qwen2.5-Coder ─────────────────────────────────────────────────
  {
    id: "qwen2.5-coder-1.5b",
    legacy: true,
    name: "Qwen2.5-Coder 1.5B Instruct",
    family: "Qwen2.5-Coder",
    params: "1.5B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "on-device", "low-latency", "multilingual"],
    weaknesses: ["reasoning", "architecture"],
    minRAM: "2GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "hybrid"],
    benchmarks: { humaneval: 43.6, swebench: 2.8, aiderPolyglot: 5.5 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-11-11",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen2.5-Coder-1.5B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "qwen2.5-coder-7b",
    legacy: true,
    name: "Qwen2.5-Coder 7B Instruct",
    family: "Qwen2.5-Coder",
    params: "7B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "instruction-following", "multilingual", "reasoning"],
    weaknesses: ["architecture"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 70.4, swebench: 12.5, aiderPolyglot: 18.2 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-11-11",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen2.5-Coder-7B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "qwen2.5-coder-14b",
    legacy: true,
    name: "Qwen2.5-Coder 14B Instruct",
    family: "Qwen2.5-Coder",
    params: "14B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "instruction-following", "reasoning", "multilingual", "math"],
    weaknesses: [],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 78.2, swebench: 16.8, aiderPolyglot: 24.6 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-11-11",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen2.5-Coder-14B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "qwen2.5-coder-32b",
    legacy: true,
    name: "Qwen2.5-Coder 32B Instruct",
    family: "Qwen2.5-Coder",
    params: "32B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "instruction-following", "reasoning", "multilingual", "math", "architecture"],
    weaknesses: [],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 92.7, swebench: 21.4, aiderPolyglot: 33.8 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2024-11-11",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen2.5-Coder-32B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 4,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },

  // ── Phi-3 ──────────────────────────────────────────────────────────
  {
    id: "phi-3-mini",
    name: "Phi-3 Mini (3.8B)",
    family: "Phi-3",
    params: "3.8B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "math", "codegen", "on-device", "instruction-following"],
    weaknesses: ["multilingual", "architecture"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 58.5, swebench: 5.6, aiderPolyglot: 8.9 },
    defaultConfig: { temperature: 0.0, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-04-23",
    provider: "Microsoft",
    url: "https://huggingface.co/microsoft/Phi-3-mini-128k-instruct",
    legacy: true,
    enterprise: {
      managedHosting: ["azure-ml", "azure-ai-studio"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "phi-3-medium",
    name: "Phi-3 Medium (14B)",
    family: "Phi-3",
    params: "14B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "math", "codegen", "instruction-following", "multilingual"],
    weaknesses: ["architecture"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 62.8, swebench: 8.9, aiderPolyglot: 14.2 },
    defaultConfig: { temperature: 0.0, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-05-21",
    provider: "Microsoft",
    url: "https://huggingface.co/microsoft/Phi-3-medium-128k-instruct",
    legacy: true,
    enterprise: {
      managedHosting: ["azure-ml", "azure-ai-studio"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── WizardCoder ────────────────────────────────────────────────────
  {
    id: "wizardcoder-15b",
    name: "WizardCoder 15B V1.0",
    family: "WizardCoder",
    params: "15B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: true,
    license: "copyleft",
    commercialUse: false,
    strengths: ["codegen", "instruction-following"],
    weaknesses: ["reasoning", "architecture", "multilingual", "outdated"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 57.3, swebench: 4.1, aiderPolyglot: 7.8 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2023-06-14",
    provider: "WizardLM",
    url: "https://huggingface.co/WizardLM/WizardCoder-15B-V1.0",
    legacy: true,
    enterprise: {
      managedHosting: [],
      sdkQuality: 2,
      communitySize: 2,
      slaAvailable: false,
      vpcSupport: false,
      documentationQuality: 2,
    },
  },
  {
    id: "wizardcoder-33b",
    name: "WizardCoder 33B V1.1",
    family: "WizardCoder",
    params: "33B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: false,
    license: "copyleft",
    commercialUse: false,
    strengths: ["codegen", "instruction-following", "reasoning"],
    weaknesses: ["architecture", "multilingual", "outdated"],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 79.9, swebench: 7.8, aiderPolyglot: 13.5 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2023-11-09",
    provider: "WizardLM",
    url: "https://huggingface.co/WizardLM/WizardCoder-33B-V1.1",
    legacy: true,
    enterprise: {
      managedHosting: [],
      sdkQuality: 2,
      communitySize: 2,
      slaAvailable: false,
      vpcSupport: false,
      documentationQuality: 2,
    },
  },

  // ── Granite Code ───────────────────────────────────────────────────
  {
    id: "granite-code-3b",
    legacy: true,
    name: "Granite Code 3B Instruct",
    family: "Granite Code",
    params: "3B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "on-device", "enterprise", "multilingual"],
    weaknesses: ["reasoning", "architecture"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 28.0, swebench: 1.5, aiderPolyglot: 3.9 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-05-06",
    provider: "IBM",
    url: "https://huggingface.co/ibm-granite/granite-3b-code-instruct",
    enterprise: {
      managedHosting: ["ibm-watsonx"],
      sdkQuality: 4,
      communitySize: 3,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "granite-code-8b",
    legacy: true,
    name: "Granite Code 8B Instruct",
    family: "Granite Code",
    params: "8B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "enterprise", "multilingual", "instruction-following"],
    weaknesses: ["reasoning", "architecture"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 42.7, swebench: 4.8, aiderPolyglot: 8.3 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-05-06",
    provider: "IBM",
    url: "https://huggingface.co/ibm-granite/granite-8b-code-instruct",
    enterprise: {
      managedHosting: ["ibm-watsonx"],
      sdkQuality: 4,
      communitySize: 3,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "granite-code-20b",
    legacy: true,
    name: "Granite Code 20B Instruct",
    family: "Granite Code",
    params: "20B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "enterprise", "multilingual", "instruction-following", "reasoning"],
    weaknesses: ["architecture"],
    minRAM: "24GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 52.4, swebench: 7.1, aiderPolyglot: 12.0 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-05-06",
    provider: "IBM",
    url: "https://huggingface.co/ibm-granite/granite-20b-code-instruct",
    enterprise: {
      managedHosting: ["ibm-watsonx"],
      sdkQuality: 4,
      communitySize: 3,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "granite-code-34b",
    name: "Granite Code 34B Instruct",
    family: "Granite Code",
    params: "34B",
    contextWindow: 8_192,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "enterprise", "multilingual", "instruction-following", "reasoning"],
    weaknesses: ["architecture", "context-window-small"],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 58.1, swebench: 9.2, aiderPolyglot: 14.7 },
    defaultConfig: { temperature: 0.2, topP: 0.95, maxTokens: 2048 },
    releaseDate: "2024-05-06",
    provider: "IBM",
    url: "https://huggingface.co/ibm-granite/granite-34b-code-instruct",
    enterprise: {
      managedHosting: ["ibm-watsonx"],
      sdkQuality: 4,
      communitySize: 3,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── DeepSeek R1 ───────────────────────────────────────────────────
  {
    id: "deepseek-r1",
    name: "DeepSeek R1",
    family: "DeepSeek R1",
    params: "671B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "math", "instruction-following", "architecture"],
    weaknesses: ["compute-heavy", "high-memory", "latency-from-reasoning"],
    minRAM: "256GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 91.6, swebench: 44.6, aiderPolyglot: 56.9 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 8192 },
    releaseDate: "2025-01-20",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-R1",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "deepseek-r1-distill-qwen-32b",
    name: "DeepSeek R1 Distill Qwen 32B",
    family: "DeepSeek R1",
    params: "32B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "math", "instruction-following"],
    weaknesses: ["architecture"],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 85.2, swebench: 28.4, aiderPolyglot: 42.1 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 8192 },
    releaseDate: "2025-01-20",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Qwen-32B",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },
  {
    id: "deepseek-r1-distill-llama-8b",
    name: "DeepSeek R1 Distill Llama 8B",
    family: "DeepSeek R1",
    params: "8B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "on-device"],
    weaknesses: ["architecture", "specialized-code-tasks"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 68.4, swebench: 14.2, aiderPolyglot: 22.8 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2025-01-20",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-R1-Distill-Llama-8B",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 3,
    },
  },

  // ── DeepSeek V3-0324 ──────────────────────────────────────────────
  {
    id: "deepseek-v3-0324",
    name: "DeepSeek V3-0324",
    family: "DeepSeek V3",
    params: "671B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "reasoning", "math", "instruction-following", "multilingual", "architecture"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "256GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 93.4, swebench: 42.0, aiderPolyglot: 60.2 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 8192 },
    releaseDate: "2025-03-24",
    provider: "DeepSeek",
    url: "https://huggingface.co/deepseek-ai/DeepSeek-V3-0324",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 3,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── Phi-4 ─────────────────────────────────────────────────────────
  {
    id: "phi-4",
    legacy: true,
    name: "Phi-4 (14B)",
    family: "Phi-4",
    params: "14B",
    contextWindow: 16_384,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "math", "codegen", "instruction-following"],
    weaknesses: ["context-window-small", "multilingual"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 82.0, swebench: 16.5, aiderPolyglot: 28.4 },
    defaultConfig: { temperature: 0.0, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2024-12-12",
    provider: "Microsoft",
    url: "https://huggingface.co/microsoft/phi-4",
    enterprise: {
      managedHosting: ["azure-ml", "azure-ai-studio"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "phi-4-mini",
    name: "Phi-4 Mini (3.8B)",
    family: "Phi-4",
    params: "3.8B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "math", "codegen", "on-device", "low-latency"],
    weaknesses: ["architecture", "specialized-code-tasks"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 62.5, swebench: 6.8, aiderPolyglot: 14.0 },
    defaultConfig: { temperature: 0.0, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2025-02-26",
    provider: "Microsoft",
    url: "https://huggingface.co/microsoft/Phi-4-mini-instruct",
    enterprise: {
      managedHosting: ["azure-ml", "azure-ai-studio"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "phi-4-reasoning",
    name: "Phi-4 Reasoning (14B)",
    family: "Phi-4",
    params: "14B",
    contextWindow: 32_768,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "math", "codegen", "instruction-following", "architecture"],
    weaknesses: ["multilingual"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 92.0, swebench: 22.5, aiderPolyglot: 38.6 },
    defaultConfig: { temperature: 0.0, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2025-04-30",
    provider: "Microsoft",
    url: "https://huggingface.co/microsoft/Phi-4-reasoning",
    enterprise: {
      managedHosting: ["azure-ml", "azure-ai-studio"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── Gemma 3 ───────────────────────────────────────────────────────
  {
    id: "gemma-3-4b",
    name: "Gemma 3 4B Instruct",
    family: "Gemma 3",
    params: "4B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "on-device", "low-latency", "multilingual"],
    weaknesses: ["reasoning", "architecture", "codegen"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "hybrid"],
    benchmarks: { humaneval: 42.0, swebench: 3.5, aiderPolyglot: 8.2 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2025-03-12",
    provider: "Google",
    url: "https://huggingface.co/google/gemma-3-4b-it",
    enterprise: {
      managedHosting: ["google-vertex-ai"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "gemma-3-12b",
    name: "Gemma 3 12B Instruct",
    family: "Gemma 3",
    params: "12B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "codegen", "reasoning", "multilingual"],
    weaknesses: ["architecture"],
    minRAM: "16GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 68.5, swebench: 10.2, aiderPolyglot: 18.6 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2025-03-12",
    provider: "Google",
    url: "https://huggingface.co/google/gemma-3-12b-it",
    enterprise: {
      managedHosting: ["google-vertex-ai"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "gemma-3-27b",
    name: "Gemma 3 27B Instruct",
    family: "Gemma 3",
    params: "27B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "codegen", "reasoning", "multilingual", "math", "architecture"],
    weaknesses: [],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 87.8, swebench: 18.4, aiderPolyglot: 32.5 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2025-03-12",
    provider: "Google",
    url: "https://huggingface.co/google/gemma-3-27b-it",
    enterprise: {
      managedHosting: ["google-vertex-ai"],
      sdkQuality: 4,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── Mistral Small 3.1 ─────────────────────────────────────────────
  {
    id: "mistral-small-3.1",
    name: "Mistral Small 3.1 (24B)",
    family: "Mistral Small",
    params: "24B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "codegen", "multilingual", "math"],
    weaknesses: [],
    minRAM: "24GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 78.5, swebench: 18.8, aiderPolyglot: 30.5 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 4096 },
    releaseDate: "2025-03-14",
    provider: "Mistral AI",
    url: "https://huggingface.co/mistralai/Mistral-Small-3.1-24B-Instruct-2503",
    enterprise: {
      managedHosting: ["mistral-platform", "together.ai", "fireworks.ai", "aws-bedrock"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── Codestral ─────────────────────────────────────────────────────
  {
    id: "codestral-2501",
    name: "Codestral 25.01 (22B)",
    family: "Codestral",
    params: "22B",
    contextWindow: 256_000,
    quantizable: true,
    onDevice: false,
    license: "restricted",
    commercialUse: false,
    strengths: ["codegen", "fill-in-middle", "reasoning", "instruction-following", "multilingual"],
    weaknesses: ["license-restrictions"],
    minRAM: "24GB",
    typicalLatency: "medium",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 88.2, swebench: 24.5, aiderPolyglot: 36.8 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2025-01-14",
    provider: "Mistral AI",
    url: "https://huggingface.co/mistralai/Codestral-2501",
    enterprise: {
      managedHosting: ["mistral-platform"],
      sdkQuality: 5,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── Devstral ──────────────────────────────────────────────────────
  {
    id: "devstral-small-2",
    name: "Devstral Small 2 (24B)",
    family: "Devstral",
    params: "24B",
    contextWindow: 256_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "reasoning", "instruction-following", "architecture", "fill-in-middle"],
    weaknesses: [],
    minRAM: "24GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 90.4, swebench: 68.0, aiderPolyglot: 48.2 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2025-12-15",
    provider: "Mistral AI",
    url: "https://huggingface.co/mistralai/Devstral-Small-2-2512",
    enterprise: {
      managedHosting: ["mistral-platform", "together.ai"],
      sdkQuality: 5,
      communitySize: 4,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── Qwen3 ─────────────────────────────────────────────────────────
  {
    id: "qwen3-8b",
    name: "Qwen3 8B",
    family: "Qwen3",
    params: "8B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "instruction-following", "multilingual", "math"],
    weaknesses: ["architecture"],
    minRAM: "8GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 74.2, swebench: 15.8, aiderPolyglot: 26.5 },
    defaultConfig: { temperature: 0.7, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2025-04-28",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen3-8B",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "qwen3-32b",
    name: "Qwen3 32B",
    family: "Qwen3",
    params: "32B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "instruction-following", "multilingual", "math", "architecture"],
    weaknesses: [],
    minRAM: "32GB",
    typicalLatency: "medium",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 88.5, swebench: 28.6, aiderPolyglot: 42.0 },
    defaultConfig: { temperature: 0.7, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2025-04-28",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen3-32B",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "qwen3-30b-a3b",
    name: "Qwen3 30B-A3B",
    family: "Qwen3",
    params: "30B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: true,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "instruction-following", "on-device", "low-latency"],
    weaknesses: ["architecture"],
    minRAM: "4GB",
    typicalLatency: "low",
    fineTunable: true,
    adapterSupport: true,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface", "lm-studio"],
    deploymentOptions: ["local", "cloud", "hybrid"],
    benchmarks: { humaneval: 72.5, swebench: 14.2, aiderPolyglot: 24.8 },
    defaultConfig: { temperature: 0.7, topP: 0.95, maxTokens: 4096 },
    releaseDate: "2025-04-28",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen3-30B-A3B",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
  {
    id: "qwen3-235b-a22b",
    name: "Qwen3 235B-A22B",
    family: "Qwen3",
    params: "235B",
    contextWindow: 128_000,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["reasoning", "codegen", "instruction-following", "multilingual", "math", "architecture"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "128GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 94.2, swebench: 38.8, aiderPolyglot: 59.6 },
    defaultConfig: { temperature: 0.7, topP: 0.95, maxTokens: 8192 },
    releaseDate: "2025-04-28",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen3-235B-A22B",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },

  // ── Llama 4 ───────────────────────────────────────────────────────
  {
    id: "llama-4-scout",
    name: "Llama 4 Scout",
    family: "Llama 4",
    params: "109B",
    contextWindow: 524_288,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "multilingual", "codegen", "math"],
    weaknesses: ["compute-heavy"],
    minRAM: "64GB",
    typicalLatency: "medium",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["ollama", "llama.cpp", "vllm", "huggingface"],
    deploymentOptions: ["cloud", "hybrid"],
    benchmarks: { humaneval: 84.5, swebench: 22.8, aiderPolyglot: 35.2 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 8192 },
    releaseDate: "2025-04-05",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Llama-4-Scout-17B-16E-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },
  {
    id: "llama-4-maverick",
    name: "Llama 4 Maverick",
    family: "Llama 4",
    params: "400B",
    contextWindow: 1_048_576,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["instruction-following", "reasoning", "multilingual", "codegen", "math", "architecture"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "256GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 90.2, swebench: 32.5, aiderPolyglot: 48.0 },
    defaultConfig: { temperature: 0.1, topP: 0.9, maxTokens: 8192 },
    releaseDate: "2025-04-05",
    provider: "Meta",
    url: "https://huggingface.co/meta-llama/Llama-4-Maverick-17B-128E-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai", "replicate", "aws-bedrock", "azure-ml"],
      sdkQuality: 5,
      communitySize: 5,
      slaAvailable: true,
      vpcSupport: true,
      documentationQuality: 5,
    },
  },

  // ── Qwen3-Coder ───────────────────────────────────────────────────
  {
    id: "qwen3-coder-480b",
    name: "Qwen3-Coder 480B-A35B",
    family: "Qwen3-Coder",
    params: "480B",
    contextWindow: 262_144,
    quantizable: true,
    onDevice: false,
    license: "permissive",
    commercialUse: true,
    strengths: ["codegen", "reasoning", "math", "instruction-following", "architecture", "fill-in-middle"],
    weaknesses: ["compute-heavy", "high-memory"],
    minRAM: "256GB",
    typicalLatency: "high",
    fineTunable: false,
    adapterSupport: false,
    ecosystem: ["vllm", "huggingface"],
    deploymentOptions: ["cloud"],
    benchmarks: { humaneval: 96.2, swebench: 65.4, aiderPolyglot: 68.5 },
    defaultConfig: { temperature: 0.0, topP: 0.95, maxTokens: 8192 },
    releaseDate: "2025-07-22",
    provider: "Alibaba",
    url: "https://huggingface.co/Qwen/Qwen3-Coder-480B-A35B-Instruct",
    enterprise: {
      managedHosting: ["together.ai", "fireworks.ai"],
      sdkQuality: 4,
      communitySize: 5,
      slaAvailable: false,
      vpcSupport: true,
      documentationQuality: 4,
    },
  },
];

// Helper: parse "8GB" → 8, "256GB" → 256
export function parseMemGB(memStr) {
  if (!memStr) return Infinity;
  const match = memStr.match(/^(\d+)/);
  return match ? parseInt(match[1], 10) : Infinity;
}

// Helper: get all unique families
export function getModelFamilies() {
  return [...new Set(MODELS.map((m) => m.family))];
}

// Helper: find model by id
export function getModelById(id) {
  return MODELS.find((m) => m.id === id) || null;
}
